...
//Set mongoose deprecated waring 
...
router.get('/', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  profile.findOne({ user: req.user.id })
    ...
    .then((profile) => {
      if (!profile) {
        errors.noprofile = "There is no profile matching for this user id";
...
router.get('/user/:user_id', (req, res) => {
  ...
  profile.findOne({ user: req.params.user_id })
    ...
    .then((profile) => {
      ...
      res.json({ profile: "There is no profile matched" });
    });
});
